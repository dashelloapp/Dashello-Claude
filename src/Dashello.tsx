...
// Resolve active color from traffic-light rules
function resolveColor(metric:Metric): MetricColor {
  if(!metric.colorRules||metric.colorRules.length===0) return metric.color;
  const num = parseFloat(metric.value.replace(/[^0-9.\-]/g,""));
  if(isNaN(num)) return metric.color;
  ...
  return "gray"; // no rule matched → gray
}
      ...
      <div style={{background:"#fff",borderRadius:24,width:"100%",maxWidth:900,maxHeight:"92vh",overflowY:"auto",
        ...
        {isLeads?<>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:28}}>
            ...
            <button style={{background:"#9CA3AF",color:"#fff",border:"none",borderRadius:8,padding:"10px 24px",fontSize:14,fontWeight:600,cursor:"pointer"}}>Edit Settings</button>
          </div>
...
// SVG favicon-style icons (paths rendered as inline SVG in a 20x20 viewbox)
// We store them as simple unicode symbols that render cleanly at small sizes — 
// organised into categories for the scrollable picker
const ICON_NONE = "";
const ICONS_ALL = [
  // Finance
  "💳","💰","💵","💴","💶","💷","🏦","📈","📉","📊","🪙","💎",
...
function MetricBoxSettingsModal({initial,onSave,onDelete,onClose}:{
  ...
  const handleSave=()=>{
    ...
    const baseColor:MetricColor="gray"; // always gray; traffic light rules drive color
        ...
        <div style={{padding:"8px 24px 24px"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:16}}>
              ...
              {/* Icon picker — scrollable, 3 rows tall */}
              <div>
                ...
                {/* Scrollable grid — fixed 3-row height */}
...
function EditAddRowModal({initial,onSave,onClose}:{initial?:string;onSave:(name:string)=>void;onClose:()=>void}) {
  ...
  return(
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:3000}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:20,padding:"28px 28px 24px",width:320,boxShadow:"0 20px 60px rgba(0,0,0,0.18)"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24}}>
...
function AddTeamModal({onClose}:{onClose:()=>void}) {
  ...
  return(
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:3000}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:20,padding:"32px",width:480,boxShadow:"0 24px 64px rgba(0,0,0,0.18)"}}>
        ...
        <p style={{margin:"0 0 24px",fontSize:13,color:"#94a3b8",textAlign:"center",lineHeight:1.5}}>
          You can set permission levels for each team member, and give access to<br/>different metrics to only the people that need to see them.
        </p>
        {members.map((m,i)=>(
...
function GoalsPage() {
  const goals=[
    {label:"Annual Sales Goal",value:"$1,200,000",current:"$235,000",pct:20,color:"#4CAF7D"},
    {label:"Monthly Revenue",value:"$100,000",current:"$79,941",pct:80,color:"#4C9FE8"},
    {label:"Lead Conversion Rate",value:"50%",current:"24%",pct:48,color:"#F5A623"},
    {label:"Website Engagement",value:"85%",current:"67%",pct:79,color:"#4CAF7D"},
    {label:"Email Open Rate",value:"40%",current:"26%",pct:65,color:"#E85D75"},
  ];
  const [selected,setSelected]=useState<number|null>(null);
  return(
    <div style={{padding:"32px"}}>
      <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:28}}>
        <h1 style={{margin:0,fontSize:26,fontWeight:700,color:"#1a2332"}}>Industry Goals</h1>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,marginBottom:28}}>
        {goals.map((g,i)=>(
          <div key={i} onClick={()=>setSelected(i===selected?null:i)}
            style={{background:"#fff",borderRadius:16,padding:20,border:selected===i?"2px solid #3B82F6":"2px solid #f1f5f9",cursor:"pointer",transition:"border 0.15s"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
              <div style={{fontSize:15,fontWeight:600,color:"#1a2332"}}>{g.label}</div>
              <div style={{fontSize:13,color:"#94a3b8"}}>{g.current} / {g.value}</div>
            </div>
            <div style={{height:10,borderRadius:99,background:"#e5e7eb",overflow:"hidden"}}>
              <div style={{width:`${g.pct}%`,height:"100%",borderRadius:99,background:g.color,transition:"width 0.4s"}}/>
            </div>
            <div style={{fontSize:12,color:"#94a3b8",marginTop:6}}>{g.pct}% complete</div>
          </div>
        ))}
      </div>

      <div style={{background:"#fff",borderRadius:16,padding:24,border:"1px solid #f1f5f9"}}>
        <h2 style={{margin:"0 0 20px",fontSize:18,fontWeight:700,color:"#1a2332"}}>Company Goals</h2>
        <div style={{display:"grid",gridTemplateColumns:"1fr 2fr",gap:24,alignItems:"start"}}>
          <div>
            <div style={{fontSize:13,color:"#94a3b8",marginBottom:4}}>Revenue Target 2025</div>
            <div style={{fontSize:24,fontWeight:700,color:"#1a2332",marginBottom:16}}>$1,200,000</div>
            <div style={{fontSize:13,color:"#94a3b8",marginBottom:4}}>Life-Time Revenue Goal</div>
            <div style={{fontSize:20,fontWeight:700,color:"#1a2332",marginBottom:16}}>$10,000,000</div>
            <div style={{fontSize:13,color:"#94a3b8",marginBottom:4}}>Team Size Goal</div>
            <div style={{fontSize:20,fontWeight:700,color:"#1a2332"}}>25 employees</div>
          </div>
          <div>
            <div style={{border:"1px solid #e2e8f0",borderRadius:12,padding:"8px 12px"}}><Sparkline/></div>
            <div style={{display:"flex",gap:12,marginTop:16}}>
              {[{label:"Overhead",val:"$79,941",color:"green"},{label:"Profit",val:"$235K",color:"yellow"},
                {label:"Tax",val:"$23,750",color:"gray"},{label:"Invest",val:"$0",color:"gray"}].map((b,i)=>(
                <div key={i} style={{flex:1,background:MS[b.color as MetricColor].bg,borderRadius:12,padding:"12px",textAlign:"center"}}>
                  <div style={{fontSize:11,color:MS[b.color as MetricColor].text,fontWeight:600}}>{b.label}</div>
                  <div style={{fontSize:14,fontWeight:700,color:MS[b.color as MetricColor].text,marginTop:4}}>{b.val}</div>
                </div>
              ))}
            </div>
          </div>
...
function TasksPage() {
  ...
  return(
    <div style={{padding:"32px"}}>
      <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:28}}>
        <h1 style={{margin:0,fontSize:26,fontWeight:700,color:"#1a2332"}}>Tasks</h1>
        <button style={{padding:"8px 20px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#3B82F6,#06B6D4)",color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer",marginLeft:"auto"}}>
          + Add Task
        </button>
      </div>
        ...
        <div>
          <div style={{display:"flex",gap:10,marginBottom:20}}>
            {["All","Active","Completed"].map((f,i)=>(
              <div key={f} style={{padding:"6px 16px",borderRadius:20,fontSize:13,fontWeight:500,cursor:"pointer",
                background:i===0?"#3B82F6":"#f1f5f9",color:i===0?"#fff":"#64748b"}}>
                {f}{i===0?` (${tasks.length})`:""}
              </div>
            ))}
          </div>
          {tasks.map(t=>(
            <div key={t.id} style={{display:"flex",alignItems:"center",gap:14,padding:"14px 16px",background:"#fff",
              borderRadius:12,marginBottom:8,border:"1px solid #f1f5f9",
              ...
              <div style={{fontSize:12,color:"#94a3b8"}}>Due {t.due}</div>
  ...
  return(
    <div style={{padding:"32px"}}>
      <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:24}}>
        <h1 style={{margin:0,fontSize:26,fontWeight:700,color:"#1a2332"}}>All Apps</h1>
      ...
      <div style={{background:"#EEF9F4",border:"1px solid #c3e6d4",borderRadius:16,padding:24}}>
        ...
        <p style={{margin:"0 0 12px",fontSize:13,color:"#1e6b4e",lineHeight:1.6}}>
          ...
          Overhead (2 months expenses), Profit (6-month emergency fund), Tax (50% of surplus), Investments (50% of surplus), and Owner salary — all from one bank account.
        </p>
  ...
  return(
    <div style={{padding:"32px"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:24}}>
        ...
        <div>
          <h1 style={{margin:0,fontSize:26,fontWeight:700,color:"#1a2332"}}>{app.name}</h1>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
        ...
        <SectionCard title="Workflows">
          {["Auto-create tasks from overdue invoices","Notify team on lead stage change","Weekly summary to Slack"].map((w,i)=>(
            <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:i<2?"1px solid #f1f5f9":"none"}}>
              ...
              <span style={{fontSize:13,color:"#1a2332"}}>{w}</span>
        ...
        <SectionCard>
          <div style={{display:"inline-block",background:"#3B82F6",color:"#fff",borderRadius:99,padding:"6px 18px",fontSize:13,fontWeight:600,marginBottom:16}}>Next Actions</div>
          {[{a:"AJ"},{a:"BK"},{a:"CL"},{}].map((x,i)=>(
            <div key={i} style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
              <div style={{width:20,height:20,borderRadius:"50%",border:"1.5px solid #4CAF7D",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,color:"#4CAF7D"}}>✓</div>
              <div style={{flex:1,height:7,borderRadius:99,background:"#e2e8f0"}}/>
              <Av initials={(x as any).a}/>
            </div>
          ))}
        </SectionCard>
      </div>
...
function TeamPage() {
  ...
  return(
    <div style={{padding:"32px"}}>
      <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:28}}>
        <h1 style={{margin:0,fontSize:26,fontWeight:700,color:"#1a2332"}}>Team</h1>
...
function SettingsPage() {
  ...
  return(
    <div style={{padding:"32px",maxWidth:900}}>
      <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:28}}>
        <h1 style={{margin:0,fontSize:26,fontWeight:700,color:"#1a2332"}}>Profile</h1>
...
const NAV=[
  ...
  {icon:"⛓",label:"Integrations/Apps",page:"integrations" as Page},
...
function Sidebar({active,onNav}:{active:Page;onNav:(p:Page)=>void}) {
  return(
    <aside style={{width:185,flexShrink:0,background:"linear-gradient(160deg,#2196F3 0%,#00BCD4 100%)",
      display:"flex",flexDirection:"column",padding:"24px 12px 20px",boxShadow:"4px 0 20px rgba(33,150,243,0.2)"}}>
      ...
      <nav style={{flex:1}}>
        {NAV.map(item=>(
          <div key={item.label} onClick={()=>onNav(item.page)} style={{display:"flex",alignItems:"center",gap:8,padding:"9px 12px",borderRadius:10,marginBottom:2,cursor:"pointer",
      ...
      <div style={{color:"rgba(255,255,255,0.7)",fontSize:13,fontWeight:700,marginTop:16,textAlign:"center"}}>●● Dashello</div>
    </aside>
  );
}
...
export default function DashelloDashboard() {
  ...
  // When Edit is clicked inside the detail modal, close it and open settings
  ...
  return(
    <div style={{display:"flex",height:"100vh",fontFamily:"'Inter',sans-serif",background:"#F8FAFC"}}>
      <Sidebar active={page} onNav={p=>{setPage(p);setSelectedApp(null);}}/>
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{display:"flex",alignItems:"center",gap:10,padding:"14px 28px",borderBottom:"1px solid #E8EDF2",background:"#fff",flexShrink:0}}>
          {page==="home"&&<>
            <div style={{display:"flex",borderRadius:8,border:"1px solid #e2e8f0",overflow:"hidden"}}>
              {["Row","Column"].map((lbl,i)=>(
                <div key={lbl} style={{padding:"6px 18px",fontSize:13,fontWeight:500,cursor:"pointer",
                  background:i===0?"#3B82F6":"#fff",color:i===0?"#fff":"#94a3b8"}}>{lbl}</div>
              ))}
            </div>
          </>}
          <div style={{flex:1}}/>
          <div style={{padding:"7px 18px",borderRadius:20,border:"1px solid #e2e8f0",fontSize:13,color:"#64748b",cursor:"pointer"}}>Chat</div>
          <div style={{padding:"8px 22px",borderRadius:8,background:"linear-gradient(135deg,#3B82F6,#06B6D4)",color:"#fff",fontSize:13,fontWeight:600,cursor:"pointer"}}>Customize</div>
        </div>
      ...
      {/* Standalone settings modal opened from Edit button inside detail modal */}
